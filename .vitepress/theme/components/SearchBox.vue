...
const list = computed(() => new Set(index.search(debouncedSearchText.value).map((r) => replace(get(r, 'id', ''), /.html#(.*)$/g, ''))));
...
<template>
  <div>
    <div
      class="border h-10 px-2 rounded-md group border-gray-400 flex items-center justify-between transition-shadow">
